...
def best_score(grid, duration, dictionary, score, word_scores)
  max_length = duration-1 # minus the submit button pressing time

  best_score = score

  $finish = duration == 0 && !$solution
  return score if $finish

  dictionary = available_words(dictionary, max_length)
  $solution = true if dictionary.size == 0
  dictionary.each do |word|
    new_score = best_score(grid, duration - word.size - 1, dictionary - [word], score + word_scores[word], word_scores)
    return new_score if $finish
...
test_cases.each do |test_case|
  ...
  dictionary.sort_by!{ |w| -(1.0*word_scores[w]/w.size) }

  puts best_score(grid, duration, dictionary, 0, word_scores)
end
