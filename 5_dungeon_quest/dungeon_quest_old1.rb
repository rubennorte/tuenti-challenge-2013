...
def maximum_value(pos, seconds, gems, path=[], best_path=[], prev_pos=nil)
  $iterations += 1

  if $iterations % 1000 == 0
    puts "Iteracion #{$iterations}"
  end
  ...
  # bound
  value = gems_with_cost.reduce(0) do |memo, gem_with_cost|
    memo + gem_with_cost[1]
  end
  value += value_of(path)
  return nil if value <= value_of(best_path)
  ...
  gems_with_cost.each do |gem_with_cost|
    ...
    possible_arrival_cells(pos, prev_pos, gem).each do |pos|
      new_result = maximum_value(new_pos, new_seconds, new_gems, new_path, result, pos)
